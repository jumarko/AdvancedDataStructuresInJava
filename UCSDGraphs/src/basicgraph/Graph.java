...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		for (int vertex = 0; vertex < getNumVertices(); vertex++) {
			final List<Integer> outNeighbors = getNeighbors(vertex);
			final List<Integer> inNeighbors = getInNeighbors(vertex);
			degrees.add(outNeighbors.size() + inNeighbors.size());
		}

		// sort from largest to smallest
		degrees.sort((d1, d2) -> d2.compareTo(d1));
		return degrees;
	}
...
//		GraphLoader.loadRoadMap("data/maps/Brno_stred.map", graphFromFile);
